...
|---|---|---|
| 2026-08-22 | **Sprint 39** — **die rote CI ist namentlich gemessen und geschlossen, und der eigentliche Befund war größer als sie.** `T-0074` → `in_review`: nachgestellter CI-Checkout mit **genau** den drei Repos des Workflows — **1616 Tests / 31 rot → 1625 / 0 rot**. Alle 31 aus **einer** Familie: Zusicherungen, die am echten Bestand messen und in der CI nichts zu lesen haben. ⚠⚠ **Drei der Dateien HATTEN einen Wächter, und er fragte `process` — das die CI von `platform` MIT auscheckt.** Derselbe Fehler, den Sprint 38 an zwei Dateien vor dem Push selbst gefunden hatte, stand hier 18 Dateien tief im Bestand. **`SWR-221`**: jede der **21 Klassen / 76 Zusicherungen** benennt ihre **eigene** Eingabe; übersprungen wird genau dann, wenn DIESE fehlt — `am_bestand()` ohne Eingabe wirft. **`SWR-222`** (Gegenprobe): am vollständigen Bestand fehlt keine deklarierte Eingabe — **und sie war beim ersten Lauf rot gegen die Deklaration dieses Laufs**; gefunden hat den Fehler die Gegenprobe, nicht der Autor. **`SWR-223`**: dieselbe Bauform in JS. **`SWR-226`**: die Plantabelle wird an ihren **Spalten** erkannt statt an ihrer **Stelle** — ⚠⚠ derselbe Fehler wie in Sprint 37, in derselben Datei, obwohl die Lehre im Bericht stand; wieder gefunden nur an `nicht_geplant`. ⚠⚠ **Von den 76 Zusicherungen waren 31 rot und 45 GRÜN, ohne dass ihre Eingabe da war** — die teurere Hälfte. **`T-0073`/`T-0068`/`T-0066` → `done`** (QM-Review mit eigenen Mutationsproben). **`T-0069` und `T-0055` zurück auf `open`**: die eine DoD ist nicht bearbeitet worden, das andere Ticket hat Teil C als Teil A verbucht. **`T-0075` neu**: die Betriebsschicht dieses Hauses liegt in **keinem Repo** — 27 Dateien, 88 kB, darunter `waechter.py` und `abschluss.cmd`. | T-0074, SWR-221/222/223/226, T-0075, L-2026-08-22n/o |
| 2026-08-22 | **Sprint 38** — **der Betriebsblocker ist gefallen, und er war ein Falschbefund des eigenen Hauses.** `T-0073` → `in_review` (**`SWR-217`**): gemessen über **77** Auto-Abschlüsse des Hosts seit dem 21.08. 20:44 — **kein einziger** erreichte je `PREFLIGHT: 0`, **23** brachen an der Lock-Zeile ab. ⚠⚠ `raeume_locks` zählte die `index.lock` als Befund, während `repo_status` **160 Zeilen tiefer im selben Lauf** dasselbe Artefakt ausdrücklich als „kein Befund“ führt — **abgebrochen ist der Lauf an dem Pfad OHNE die Begründung**, und alle 7 Repos meldeten in derselben Sekunde `sauber`. Geräumt wird jetzt nach dem **Alter** des Artefakts (`LOCK_GNADENFRIST_S = 90`) statt nach der **Prozessliste des Geräts** — die Geräte-Frage lautet auf diesem Host praktisch immer „ja“, die Räumung konnte aus `abschluss.cmd` heraus **nie** greifen. ⚠ Der Eingriff ist schmal: ohne sichtbaren git-Prozess bleibt alles beim Alten, sonst verlöre `--nur-locks` seinen Zweck — in diesem Lauf **live gebraucht**. **`SWR-218`** (Nebenbefund, der sechs eigene Messungen widerlegte): ein `__pycache__/*.pyc` galt als gültig, obwohl sein Bytecode von einer Mutationsprobe stammte — gleiche Größe, gleiche `mtime`-Sekunde, auf diesem Mount nicht löschbar (R7). **Dieselbe Strecke meldete aus der Quelle `OK` und aus dem Cache `FAILED (1)` bei byte-identischer Datei.** **`SWR-220`** (aus `pm/T-0079`): uneinheitliche Besetzung wird namentlich sichtbar, ohne Befundzähler. Dazu `sicht_takt` im Post-Widget (Vertrag v2.9). | T-0073, SWR-217/218/220, L-2026-08-22i/j/k |
| 2026-08-22 | **Sprint 37** — **drei Anforderungen, 27 Zusicherungen, und zweimal dieselbe Ursache.** `T-0066` → `in_review` (**`SWR-214`**): der leere Ollama-Takt wird jetzt im **Preflight** gemeldet statt 87 Läufe lang im Protokoll eines Dienstes. ⚠⚠ Die maßgebliche Auflösung ist **gemessen, nicht gewählt** — nur `besetzungen.yaml` steht im Pfad, der ein Ticket verwirft (`tick.waehle_ticket`); `registry.yaml` wird erst **danach** gelesen, `--projekt` ist ein Aufrufargument. Gezählt über alle 383 Ticketdateien: **6 von 8 Aufgaben-Typen mit Ollama-Kette haben nie ein Ticket getragen**, 4 von 383 Tickets setzen überhaupt `aufgaben_typ`. `T-0055` Teil A → `in_review` (**`SWR-215`**): der Herzschlag des Wächters bekommt einen **Leser** — `waechter.py` begründet Frage 4 im eigenen Kopf mit *„für jeden Leser messbar"*, und der Wert blieb 14 h aus, ohne dass ihn jemand las. **`SWR-216`** (aus `pm/T-0085`): Projektkennungen aus **beiden** Quellen. ⚠ Beide Zeilen sind bewusst **kein Befund** — ein toter Wächter und ein leerer Takt dürfen den Push nicht anhalten (`SWR-166`). ⚠ Eine Mutationsprobe hat den eigenen Test widerlegt (`L-2026-08-22g`). **Und der Wächter lebt wieder:** Herzschlag 14:27:21, PID 23284. | T-0066, T-0055, SWR-214/215/216 |
| 2026-08-22 | **Sprint 36** — **`T-0060` geschlossen: der Ollama-Nachweis war seit dem 21.08. 20:59 GEFÜHRT und lag ungelesen im Register** (`status: ok`, `gemma3:27b`, 2 Artefakte, 221,2 s). Entsperrt vier Tickets. **`T-0068`**: Vorabfragen gezählt (**1** Fundstelle, **0 von 75** Belegen fälschlich durchgelassen) und die **drei ungeprüften Zweige** der Host-Reparatur mit einem Zusicherungs-Paar geschlossen (Mutationsprobe: 5 rot gegen alten Code). **`T-0055` vierte Berührung → geschnitten**: `waechter.py` ist gebaut — ⚠⚠ und **selbst seit 14 h tot**, während die Dienste laufen, die er bewacht. ⚠ Teststrecke **109 Module / 1553 Zusicherungen**; **1 rot, selbst verursacht** (`open -> in_review` ohne Zwischenschritt, Commit `04da965`). | T-0060, T-0068, T-0055, L-2026-08-22a/b/d |
...
| 2026-08-21 | **Lauf ohne Sprint (vierter ohne Shell) — die Teststrecke ist zum ERSTEN MAL seit vier Sprints vollständig gelaufen, und sie ist rot.** Nicht hier gemessen, sondern beim Host: `abschluss-logs/abschluss-20260821-211400.log` meldet **1551 Tests / 6 rot** in 210 s. ⚠⚠ Sprint 35 hatte „1551" als Verifikationszeile geführt und im selben Bericht eingeräumt, dass **1136 davon nie liefen** — die Zahl der GESAMMELTEN Zusicherungen ist vier Sprints lang als Zahl der grünen gewandert. Die sechs auf drei Ursachen zurückgeführt und terminiert: `T-0068` (`os.path.isabs` — die Schranke gegen Belege außerhalb des Bestands steht auf dem Host OFFEN, `/etc/passwd` wird durchgewinkt; Code krank, Test war grün), `T-0069` (zwei Zusicherungen messen die **Prozessliste** des Rechners statt den Schreibweg), `T-0070` (**acht** Lehren ohne Vertreter — ⚠ **sechs davon aus Sprint 35 selbst**, der die Prüfung nur nicht gefahren hat; die Erstfassung schrieb „alle acht aus Läufen ohne Shell" und wurde vom Gegenlesen widerlegt). ⚠ Dritte Zusicherung in zwei Sprints, deren Grün von der **Umgebung** des Läufers abhängt (Zeitzone → Betriebssystem → Prozessliste), und jedes Mal gefunden, weil jemand die Strecke woanders laufen ließ. ⚠⚠ **Beweis frei Haus:** im Folgelauf 21:55 sind die zwei `git_schreibweg`-Zusicherungen **ohne jede Codeänderung grün** (6 rot → 5 rot) — und **eine neue ist rot, verursacht von diesem Lauf** (`test_berichtskennzahlen`: `tickets_offen 34→38`), benannt statt von Hand nachgetragen. **Keine Zeile Code geändert** — ohne Teststrecke wäre jede Reparatur eine Behauptung. | T-0068/T-0069/T-0070, `abschluss-logs/*`, pm/T-0086 |

## Lessons Learned (Auswahl mit Verbleib — vollständige Lehren in den Rollenkarten v2)

| # | Lehre | Quelle | Übernommen nach |
|---|---|---|---|
| 22 | Zwei Prüfungen desselben Hauses über dieselbe Sache dürfen sich nicht widersprechen — 77 Läufe brachen an dem Pfad ohne Begründung ab | SWR-217 | Rollenkarte DEV, `L-2026-08-22i` |
| 23 | Eine Frage über das Gerät kann eine Frage über eine Datei nicht beantworten | SWR-217 | Rollenkarte DEV, `L-2026-08-22i` |
| 24 | Ein Versand, der nur an einer fehlenden Umgebungsvariablen scheitert, ist ein scharfer mit Glück | SWR-219 | Rollenkarte DEV, `L-2026-08-22j` |
| 25 | Eine Zusicherung, die eine andere Fassung misst als die im Repo, sieht genauso grün aus wie eine, die stimmt | SWR-218 | Rollenkarte TEST, `L-2026-08-22k` |
| 26 | Eine Probe, die den Bau zerstört statt die Regel zu verfälschen, misst nichts | SWR-220 | Rollenkarte TEST, `L-2026-08-22k` |
| 27 | Die Frage des Auftraggebers und die Ursache seines Problems sind zu messen, bevor man sie beantwortet | pm/B061 | Rollenkarte PL, `L-2026-08-22l` |
| 28 | Ein Vertragsfeld gehört im selben Lauf geliefert, in dem es zugesagt wird | SWR-131 | Rollenkarte CM, `L-2026-08-22m` |
